...
window.addEventListener("DOMContentLoaded", () => {
  ...
  function setupPicker(picker) {
    ...
    mount.innerHTML = `
      <div class="member-picker-toolbar">
        ...
        <button type="button" class="member-picker-clear">명단 선택 해제</button>
      </div>
    ...
    mount.querySelector(".member-picker-clear").addEventListener("click", () => clearRosterNames(kind, status));
  ...
  function clearRosterNames(kind, status) {
    const values = currentValues(kind).filter((name) => !roster.includes(name));
    if (kind === "wheel") updateWheel(values);
    else applyUniqueValues(kind, values);
    status.textContent = "명단에서 선택한 이름을 모두 뺐어요.";
  }
